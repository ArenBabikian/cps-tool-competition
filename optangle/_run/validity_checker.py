...
class ValidityChecker():
    ...
    def simpletest(self):
        ...
        print(individual)
        self.makeAndVisualiseTest(individual)


    def simplertest(self):

        angles = [-30, 45, 75, 75, -45]
        individual = {}
        individual['num_points'] = len(angles)
        for i, a in enumerate(angles):
            individual[f'p{i}_theta'] = a

        self.makeAndVisualiseTest(individual)
        

    def makeAndVisualiseTest(self, individual,save=False):

        # make test and run
        road_points, missing_dist = utils.getRoadPointsFromAngles(individual, self.map_size)
        assert road_points is not None, "Invlid spec: impossible to reframe"
        ...
        debug.visualise(self.executor, the_test)
        debug.visualise_simple(self.executor, the_test)
        plt.pause(5)
